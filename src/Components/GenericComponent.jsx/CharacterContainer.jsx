...
export const CharacterContainer = ({image, style, blurredImage}) => {
  return (
    ...
    <div className="border-2 border-secondary absolute -left-3 p-[20px]xl:p-24 rounded-[50px] w-full h-full blur-sm">
    </div>


    <div className="relative flex justify-center align-end h-full">
      <img  className={`p-[20px] rounded-card xl:p-24 absolute top-0  h-full blur-sm  ${style}`} src={image} alt="" />
      <img className={`xl:p-24  h-full backdrop-blur-sm  ${blurredImage}`} src={image} alt="" />
    </div>
